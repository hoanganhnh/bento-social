...
export default function NewPost({ onBack }: INewPostProps) {
    ...
    getTopics()
      .then((response) => {
        if (
          ...
          setError('No topics available. Please try again later.');
        }
      })
      .catch((error) => {
  ...
  if (error)
    return (
      <div className="fixed w-full h-full top-0 left-0 bg-[#444444] z-20 md:bg-[#12121299] flex items-center justify-center">
        <div className="bg-[#282828b3] backdrop-blur-[50px] p-6 rounded-button border border-[#ffffff1a]">
          <Typography level="base2sm" className="text-red-400 mb-4">
            {error}
          </Typography>
          <Button
            className="px-[1.5rem] py-[0.75rem] rounded-[2rem] text-secondary"
            child={<Typography level="base2sm">Go Back</Typography>}
            onClick={onBack}
          />
        </div>
      </div>
